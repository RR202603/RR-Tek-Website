const newsDetailData = {
    "2026-05-18.html": {
        date: "2026-05-18",
        title: {
            zh: "公司技术人员参加第二届“硬质材料基础科学与关键技术”学术研讨会并推广公司软件",
            en: "RR-Tek Technical Staff Attended the Second Academic Seminar on Basic Science and Key Technologies of Hard Materials and Promoted Company Software"
        },
        body: {
            zh: `
<p>2026年5月17日至19日，第二届“硬质材料基础科学与关键技术”学术研讨会在长沙召开，汇聚了来自全国及港澳高校、科研院所和企业的260余位专家学者。中国科学院院士田永君、陈光、陈延峰及中国工程院院士赵中伟出席，共同探讨硬质材料前沿科学与关键技术。</p>
<figure><img src="../assets/news-260518-img1.jpg" alt=""><figcaption>大会现场</figcaption></figure>
<p>长沙锐睿科技有限公司作为国内领先的材料智能设计软件企业，面向与会专家展示了包括ICALPHAD、CALTPP、MID-MESO等七款自主研发软件，覆盖相图热力学、微观结构演化、晶体塑性力学等材料研发全链条，实现核心算法自主可控。相关软件已服务华为、中国科学院、西北工业大学等33家企业与科研机构，助力合金设计、性能模拟及高通量研发，并累计支持发表SCI论文70余篇。</p>
<p>研讨会聚焦航空航天、信息通信等领域对高性能硬质材料的战略需求。锐睿科技技术团队展示了在人工智能辅助设计、工艺优化及多尺度性能预测等方向的应用案例，获得专家高度关注与积极反馈。</p>
<p>未来，长沙锐睿科技将继续推动材料智能设计软件发展，服务高端制造与国家科技自立，为硬质材料及高端制造领域创新贡献力量。</p>
`,
            en: `
<p>From May 17 to 19, 2026, the Second Academic Seminar on Basic Science and Key Technologies of Hard Materials was held in Changsha, bringing together more than 260 experts and scholars from universities, research institutes, and enterprises across China, including Hong Kong and Macao. Academicians Tian Yongjun, Chen Guang, Chen Yanfeng of the Chinese Academy of Sciences, and Zhao Zhongwei of the Chinese Academy of Engineering attended the seminar to discuss frontier science and key technologies for hard materials.</p>
<figure><img src="../assets/news-260518-img1.jpg" alt=""><figcaption>Conference site</figcaption></figure>
<p>As a leading domestic materials intelligent design software company, Changsha RR-Tek Technology Co., Ltd. presented seven independently developed software products, including ICALPHAD, CALTPP, and MID-MESO. These tools cover the full materials R&D chain, including phase diagram thermodynamics, microstructure evolution, and crystal plasticity mechanics, with independently controllable core algorithms. The software has served 33 enterprises and research institutions, including Huawei, the Chinese Academy of Sciences, and Northwestern Polytechnical University, supporting alloy design, property simulation, high-throughput R&D, and more than 70 SCI publications.</p>
<p>The seminar focused on strategic demand for high-performance hard materials in aerospace, information and communications, and related fields. RR-Tek's technical team presented application cases in AI-assisted design, process optimization, and multiscale property prediction, receiving strong attention and positive feedback from experts.</p>
<p>In the future, RR-Tek will continue advancing materials intelligent design software, serving high-end manufacturing and national scientific and technological self-reliance, and contributing to innovation in hard materials and advanced manufacturing.</p>
`
        }
    },
    "2026-05-17.html": {
        date: "2026-05-17",
        title: {
            zh: "公司技术人员受邀参加中国稀土学会第七届青年学术会议并作报告",
            en: "RR-Tek Technical Staff Invited to the 7th Youth Academic Conference of the Chinese Society of Rare Earths"
        },
        body: {
            zh: `
<p>近日，中国稀土学会第七届青年学术会议在江西南昌召开。会议汇聚了稀土领域高校、科研院所及企业代表，围绕稀土材料、稀土永磁、稀土功能材料、绿色冶金与应用技术等方向开展学术交流。</p>
<figure><img src="../assets/news-260517-img1.jpg" alt=""><figcaption>大会现场</figcaption></figure>
<p>公司两位技术人员受邀参加会议，并分别作邀请报告。其中，《稀土永磁体热力学数据库：从“经验试错”到“预测设计”》报告围绕稀土永磁材料研发中的热力学数据库建设与应用展开，介绍了基于热力学计算开展材料设计、组织调控及性能优化的方法与实践，展示了计算驱动材料研发在提升研发效率、降低试错成本方面的重要作用。</p>
<p>另一场邀请报告《ICALPHAD软件：相图计算与智能热力学优化的集成平台》重点介绍了ICALPHAD软件在相图计算、热力学参数优化及智能化材料设计中的功能与应用进展，展示了公司在CALPHAD方法、热力学计算平台开发以及智能材料设计方向上的技术积累。</p>
<figure><img src="../assets/news-260517-img2.jpg" alt=""><figcaption>公司技术人员作题为《稀土永磁体热力学数据库：从“经验试错”到“预测设计”》的邀请报告</figcaption></figure>
<figure><img src="../assets/news-260517-img3.jpg" alt=""><figcaption>公司技术人员作题为《ICALPHAD软件：相图计算与智能热力学优化的集成平台》的邀请报告</figcaption></figure>
<p>会议期间，公司参会人员还与相关领域专家学者围绕稀土永磁材料设计、热力学数据库开发及产业应用等内容进行了深入交流，进一步了解行业前沿技术与发展趋势。</p>
<p>此次受邀作报告，体现了行业对公司在稀土永磁材料热力学研究与计算辅助设计领域技术工作的认可。未来，公司将继续加强技术创新与产学研交流合作，持续提升核心技术能力，为稀土功能材料产业高质量发展贡献力量。</p>
`,
            en: `
<p>Recently, the 7th Youth Academic Conference of the Chinese Society of Rare Earths was held in Nanchang, Jiangxi. The conference brought together representatives from universities, research institutes, and enterprises in the rare-earth field for academic exchange on rare-earth materials, rare-earth permanent magnets, functional rare-earth materials, green metallurgy, and application technologies.</p>
<figure><img src="../assets/news-260517-img1.jpg" alt=""><figcaption>Conference site</figcaption></figure>
<p>Two technical staff members from RR-Tek were invited to attend and deliver invited talks. The report titled "Rare-Earth Permanent Magnet Thermodynamic Database: From Empirical Trial-and-Error to Predictive Design" focused on thermodynamic database development and application in rare-earth permanent magnet R&D, introducing methods and practices for materials design, microstructure control, and property optimization based on thermodynamic calculations.</p>
<p>Another invited talk, "ICALPHAD Software: An Integrated Platform for Phase Diagram Calculation and Intelligent Thermodynamic Optimization," introduced the functions and application progress of ICALPHAD in phase diagram calculation, thermodynamic parameter optimization, and intelligent materials design, demonstrating RR-Tek's technical accumulation in CALPHAD methods, thermodynamic computing platform development, and intelligent materials design.</p>
<figure><img src="../assets/news-260517-img2.jpg" alt=""><figcaption>RR-Tek technical staff delivering the invited talk on rare-earth permanent magnet thermodynamic databases</figcaption></figure>
<figure><img src="../assets/news-260517-img3.jpg" alt=""><figcaption>RR-Tek technical staff delivering the invited talk on ICALPHAD software</figcaption></figure>
<p>During the conference, RR-Tek participants also held in-depth exchanges with experts and scholars on rare-earth permanent magnet design, thermodynamic database development, and industrial applications, gaining further insight into industry frontiers and development trends.</p>
<p>This invitation reflects industry recognition of RR-Tek's work in rare-earth permanent magnet thermodynamics and computation-assisted design. RR-Tek will continue strengthening technological innovation and industry-university-research collaboration to support high-quality development of rare-earth functional materials.</p>
`
        }
    },
    "2026-04-24.html": {
        date: "2026-04-24",
        title: {
            zh: "锐睿科技发布7款材料智能设计软件，构建国产工业软件全链条能力",
            en: "RR-Tek Released Seven Materials Intelligent Design Software Products, Building Full-Chain Domestic Industrial Software Capabilities"
        },
        body: {
            zh: null,
            en: `
<p>On April 24, 2026, Changsha RR-Tek Technology Co., Ltd. successfully held the Materials Intelligent Design Software Product Launch Conference at Wyndham Garden Hotel in Yuelu University Town. The company officially released seven independently developed materials intelligent design software products for industry users, covering phase diagram thermodynamic calculation, thermophysical property calculation, diffusion kinetics, microstructure simulation, and crystal plasticity analysis. The release marks an important step toward a full-chain independent solution for materials computational industrial software in China.</p>
<p>The conference brought together leaders, experts, and representatives from government, universities, research institutes, and industry. Several speakers noted that materials computation software has long depended on overseas products, often facing algorithmic black boxes and closed databases. RR-Tek's released software system makes progress in independent algorithms, self-developed databases, and engineering application, and is significant for high-end manufacturing and new materials development.</p>
<figure><img src="../assets/news-260424-img1.jpg" alt=""><figcaption>Speech by Liu Weidong, First-Level Inspector of the Hunan Provincial Department of Industry and Information Technology</figcaption></figure>
<figure><img src="../assets/news-260424-img2.jpg" alt=""><figcaption>Speech by Professor George Kaptay, Academician of the Hungarian Academy of Sciences</figcaption></figure>
<figure><img src="../assets/news-260424-img3.jpg" alt=""><figcaption>Speech by Zhang Zhongjian, President of the Zhuzhou Cemented Carbide Association</figcaption></figure>
<figure><img src="../assets/news-260424-img4.jpg" alt=""><figcaption>Speech by Professor Du Yong, Academician of the Hungarian Academy of Sciences</figcaption></figure>
<figure><img src="../assets/news-260424-img5.jpg" alt=""><figcaption>Software launch ceremony</figcaption></figure>
<p>The seven released products build a complete technical system: ICALPHAD for phase diagrams and thermodynamics; CALTPP diffusion, viscosity, and thermal conductivity modules; MID-NANO for nanoscale microstructure simulation; MID-MESO for mesoscale multiphysics microstructure simulation; and CPCP for crystal plasticity finite element calculation.</p>
<p>These products support tasks such as multicomponent phase diagram calculation, intelligent thermodynamic parameter optimization, diffusion and mobility calculation, liquid viscosity prediction, thermal conductivity calculation, phase-field simulation, and multiscale mechanics simulation. Together, they provide software support across composition design, process optimization, microstructure evolution, and performance prediction.</p>
<figure><img src="../assets/news-260424-img6.jpg" alt=""><figcaption>Software demonstration and technical exchange</figcaption></figure>
<figure><img src="../assets/news-260424-img7.jpg" alt=""><figcaption>Discussion with participating experts</figcaption></figure>
<figure><img src="../assets/news-260424-img8.jpg" alt=""><figcaption>Conference participants</figcaption></figure>
<p>RR-Tek will continue focusing on core algorithm development, scientific database construction, and engineering application, helping materials R&D move from experience-driven trial-and-error toward computation-guided intelligent design.</p>
`
        }
    },
    "2025-03-05.html": {
        date: "2025-03-05",
        title: {
            zh: "重大喜讯！锐睿科技荣获高新技术企业认定",
            en: "Great News! RR-Tek Has Been Recognized as a High-Tech Enterprise"
        },
        body: {
            zh: `
<p>2025年3月5日，高企认定官网披露了对湖南省认定机构 2024 年认定报备的高新技术企业进行第一批补充备案的公告，锐睿科技赫然在列。这一认定不仅是对我司过往成绩的高度认可，更标志着我司在技术创新和企业发展道路上迈出了坚实而有力的一步。</p>
<p>自 2021 年成立以来，我们始终秉持创新驱动的发展理念，深耕于科技推广和应用服务领域。尽管成立时间不长，但公司已展现出了良好的发展潜力。近两年参与大型招投标项目 2 次，拥有实用新型专利 2 项、软件著作权 10 件。这些成果的背后，是锐睿科技团队持续钻研与不懈拼搏的体现，也是公司对技术创新长期投入的结果。</p>
<p>展望未来，公司将以此次高企认定为新的起点，继续坚持科技创新驱动发展战略，不断提升自身核心竞争力。我们将积极探索更加高效的科技推广和应用服务模式，深化在新材料软件开发及服务研究方面的布局，致力于打造具有国际竞争力的科技企业形象。</p>
<p>同时，公司也将积极与高校、研究院所开展合作，构建开放的技术创新平台，实现资源共享和优势互补，为推动区域乃至国家的科技进步贡献更多力量。</p>
`,
            en: `
<p>On March 5, 2025, the official high-tech enterprise recognition platform released the first supplementary filing announcement for high-tech enterprises reported by Hunan Province's recognition authority in 2024, and RR-Tek was included. This recognition is not only a strong affirmation of the company's past achievements, but also marks a solid step forward in technological innovation and enterprise development.</p>
<p>Since its establishment in 2021, RR-Tek has adhered to innovation-driven development and focused on technology promotion and application services. Although still a young company, RR-Tek has shown strong development potential. In recent years, the company has participated in two major bidding projects and obtained two utility model patents and ten software copyrights.</p>
<p>Looking ahead, RR-Tek will take this recognition as a new starting point, continue implementing an innovation-driven development strategy, and further strengthen its core competitiveness. The company will explore more efficient technology promotion and application service models, deepen its layout in new materials software development and services, and build an internationally competitive technology enterprise image.</p>
<p>RR-Tek will also actively collaborate with universities and research institutes to build open technological innovation platforms, share resources, complement strengths, and contribute to regional and national scientific and technological progress.</p>
`
        }
    },
    "2024-10-09.html": {
        date: "2024-10-09",
        title: {
            zh: "材料领域工业软件和大数据产学研用研讨会于9月20日至23日在苏州顺利举办",
            en: "Seminar on Industrial Software and Big Data for Materials Successfully Held in Suzhou"
        },
        body: {
            zh: `
<p>2024年9月20日至23日，由长三角先进材料研究院、中南大学、山东大学、苏州臻材科技有限公司联合主办的“材料领域工业软件和大数据产学研用研讨会”在江苏苏州顺利举办。来自国内高等院校、科研院所及相关企业超160余名专家学者、企业代表与会，共同探讨了材料领域工业软件及大数据的发展现状与未来趋势。本次研讨会共有10个特邀报告，70个口头报告，24个海报。</p>
<p>9月21日上午，材料领域工业软件和大数据产学研用研讨会开幕式在苏州举行。中南大学杜勇教授为大会开幕式致辞。</p>
<p>会上，罗宏杰教授作大会主旨报告，介绍了长三角先进材料研究院材料基因组工程研究现状与展望。多位专家分别作大会特邀报告，分享材料模拟、数据分析与智能制造等领域的最新进展。</p>
<p>本次研讨会涵盖材料模拟与仿真、数据分析与挖掘、智能化制造等多个主题。与会代表积极交流并提出了许多有价值的观点和建议，推动了材料领域工业软件和大数据产学研用的深度融合。</p>
`,
            en: `
<p>From September 20 to 23, 2024, the Seminar on Industrial Software and Big Data for Materials was successfully held in Suzhou, Jiangsu. The event was jointly organized by the Yangtze River Delta Advanced Materials Research Institute, Central South University, Shandong University, and Suzhou Zhencai Technology Co., Ltd. More than 160 experts, scholars, and enterprise representatives attended to discuss the development status and future trends of industrial software and big data in materials. The seminar featured 10 invited talks, 70 oral presentations, and 24 posters.</p>
<p>On the morning of September 21, the opening ceremony was held in Suzhou, with Professor Du Yong of Central South University delivering the opening remarks.</p>
<p>Professor Luo Hongjie delivered a keynote report on the current status and outlook of materials genome engineering at the Yangtze River Delta Advanced Materials Research Institute. Other invited experts shared recent progress in materials simulation, data analysis, and intelligent manufacturing.</p>
<p>The seminar covered topics such as materials modeling and simulation, data analysis and mining, and intelligent manufacturing. Participants held active discussions and offered valuable views and suggestions, promoting deeper integration of industry, academia, research, and application in industrial software and big data for materials.</p>
`
        }
    },
    "2023-08-10.html": {
        date: "2023-08-10",
        title: {
            zh: "我公司成功为国内某大型央企交付相场软件",
            en: "RR-Tek Successfully Delivered Phase-Field Software to a Major Central State-Owned Enterprise"
        },
        body: {
            zh: `
<p>2023年7月，锐睿科技成功为国内某大型央企交付相场软件全套设备，并完成指导安装及培训工作，为其航空材料改进提供了关键性支持。</p>
<p>自 2021 年 6 月签约该项目以来，经过研发人员近两年的持续开发、设计与反复调试，项目最终于 2023 年 7 月顺利完成交付。公司研发团队在交付后还将继续提供后期维护与技术支持服务，保障软件在实际应用中的稳定运行。</p>
<p>本次项目的成功实施，为解决和提升该企业在航空材料改性方面的关键问题提供了重要技术支撑，也体现了锐睿科技在相场模拟软件开发与工程应用服务方面的专业能力。</p>
`,
            en: `
<p>In July 2023, RR-Tek successfully delivered a complete set of phase-field software equipment to a major central state-owned enterprise in China and completed installation guidance and training, providing key support for improving aviation materials.</p>
<p>Since the project was signed in June 2021, RR-Tek's R&D team spent nearly two years on continuous development, design, and repeated debugging. The project was successfully delivered in July 2023. The R&D team will continue to provide maintenance and technical support to ensure stable operation in practical applications.</p>
<p>The successful implementation of this project provides important technical support for solving key issues in aviation material modification and demonstrates RR-Tek's professional capabilities in phase-field simulation software development and engineering application services.</p>
`
        }
    },
    "2023-04-06.html": {
        date: "2023-04-06",
        title: {
            zh: "2023年难熔金属及硬质材料国际论坛顺利举办",
            en: "2023 International Forum on Refractory Metals and Hard Materials Successfully Held"
        },
        body: {
            zh: `
<p>3月31日至4月2日，由山东大学材料科学与工程学院、中南大学粉末冶金国家重点实验室、中南大学有色金属材料与工程教育部重点实验室、湖南博云东方粉末冶金有限公司联合主办的“2023年难熔金属及硬质材料国际论坛”在湖南长沙顺利召开。</p>
<p>中国工程院院士黄伯云、科技部高技术研究发展中心技术总师史冬梅、株洲硬质合金产业协会执行会长张忠健、中南大学教授杜勇、昆明理工大学副校长易健宏、北京科技大学教授曲选辉、大连理工大学教授董闯、中国科学院金属研究所研究员徐东生、德国卡尔斯鲁厄理工学院教授 Hans Seifert、中南大学教授张立、上海大学刘轶等专家教授出席本届论坛。</p>
<p>来自国内外高等院校、科研院所及相关企业共 200 余名代表和专家与会。会议共有 10 个特邀报告、49 个口头报告和 28 个海报展示，从不同研究方向展示了难熔金属及硬质材料领域的研究成果与前沿动态。</p>
<p>4月1日上午，论坛开幕式举行。中国工程院院士黄伯云和山东大学材料科学与工程学院教授赵国群致辞。会上，史冬梅总师作大会主旨报告，介绍了数据驱动的新材料技术发展趋势。</p>
`,
            en: `
<p>From March 31 to April 2, the 2023 International Forum on Refractory Metals and Hard Materials was successfully held in Changsha, Hunan. The forum was jointly organized by the School of Materials Science and Engineering of Shandong University, the State Key Laboratory of Powder Metallurgy of Central South University, the Key Laboratory of Nonferrous Metal Materials Science and Engineering of the Ministry of Education, and Hunan Boyun-Dongfang Powder Metallurgy Co., Ltd.</p>
<p>Many experts attended the forum, including Academician Huang Boyun of the Chinese Academy of Engineering, Shi Dongmei, chief engineer of the High Technology Research and Development Center of the Ministry of Science and Technology, Zhang Zhongjian of the Zhuzhou Cemented Carbide Industry Association, Professor Du Yong of Central South University, and other scholars from universities and research institutes in China and abroad.</p>
<p>More than 200 representatives and experts from universities, research institutes, and enterprises attended. The forum included 10 invited reports, 49 oral reports, and 28 poster presentations, showcasing research progress and frontier developments in refractory metals and hard materials.</p>
<p>The opening ceremony was held on the morning of April 1. Academician Huang Boyun and Professor Zhao Guoqun of Shandong University delivered speeches. Chief Engineer Shi Dongmei gave a keynote report on data-driven development trends in new materials technology.</p>
`
        }
    },
    "2022-04-28.html": {
        date: "2022-04-28",
        title: {
            zh: "热烈祝贺我公司与奥地利 MatCalc 工程公司达成合作并签约！",
            en: "Warm Congratulations on RR-Tek's Cooperation Agreement with MatCalc Engineering GmbH, Austria"
        },
        body: {
            zh: `
<p>日前，我公司就 MatCalc 系列软件销售及为国内企业与高等科研院所提供冶金及材料工程技术解决方案，与 MatCalc Engineering GmbH 公司达成合作并签约，成为国内独家签约代理，负责代理销售 MatCalc 软件并提供相关技术解决方案。</p>
<p>MatCalc Engineering GmbH 致力于提供冶金及材料工程技术解决方案。公司由多学科专家团队负责运营，专长涵盖计算材料工程、热力学和动力学数据库研发、材料表征与测试、固态相变和析出建模，以及微观结构与性能关系研究等多个方向，致力于缩小基础研究与工业实践之间的距离。</p>
<p>该公司的经营模式将材料相关工程服务与 MatCalc 软件应用紧密结合，以软件作为中心建模和仿真工具，为客户提供系统化技术支持。</p>
<p>MatCalc 相关服务范围包括冶金加工过程中的微观结构演变预测、新材料评估、计算材料研发、过程建模和生产工序优化、跨尺度建模、材料加工咨询、先进显微分析、材料检测、故障调查、材料缺陷分析以及培训和科学研讨会服务。</p>
`,
            en: `
<p>Recently, RR-Tek reached and signed a cooperation agreement with MatCalc Engineering GmbH regarding sales of the MatCalc software series and the provision of metallurgical and materials engineering technical solutions for domestic enterprises, universities, and research institutes. RR-Tek has become the exclusive contracted agent in China for MatCalc software sales and related technical solutions.</p>
<p>MatCalc Engineering GmbH is committed to providing metallurgical and materials engineering solutions. Operated by a multidisciplinary expert team, the company specializes in computational materials engineering, thermodynamic and kinetic database development, materials characterization and testing, solid-state phase transformation and precipitation modeling, and microstructure-property relationship studies, aiming to bridge fundamental research and industrial practice.</p>
<p>The company's business model closely integrates materials engineering services with MatCalc software applications, using software as the central modeling and simulation tool to provide systematic technical support to customers.</p>
<p>MatCalc-related services include prediction of microstructure evolution during metallurgical processing, evaluation of new materials before application, computational materials R&D, process modeling and production optimization, multiscale modeling in ICME, materials and processing consulting, advanced microscopy, EBSD, DSC, materials testing, failure analysis, defect investigation, training programs, and scientific workshops.</p>
`
        }
    }
};

function renderNewsDetail(lang = "zh") {
    const page = window.location.pathname.split("/").pop();
    const data = newsDetailData[page];
    if (!data) return;

    const title = document.querySelector(".news-detail-page h1");
    const intro = document.querySelector(".news-detail-page .detail-intro");
    const article = document.querySelector(".news-article");

    if (title) {
        title.textContent = data.title[lang] || data.title.zh;
    }

    if (intro) {
        intro.textContent = `${lang === "en" ? "Published" : "发布日期"}：${data.date}`;
    }

    if (article) {
        if (!article.dataset.originalHtml) {
            article.dataset.originalHtml = article.innerHTML;
        }

        const body = lang === "en" ? data.body.en : (data.body.zh || article.dataset.originalHtml);
        if (body) {
            article.innerHTML = body;
        }
    }
}
